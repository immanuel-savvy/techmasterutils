...
var new_article = function new_article(req, res) {
  ...
  article.views = 0;
  article.sections = article.sections.map(function (sec) {
    if (sec.type === "image" && sec.image && sec.image.startsWith("data")) sec.image = (0, _utils.save_image)(sec.image);
    return sec;
  });
...
var remove_article = function remove_article(req, res) {
  ...
  result.sections.map(function (sec) {
    return sec.image && (0, _utils.remove_image)(sec.image);
  });
